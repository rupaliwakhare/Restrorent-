    ...
    <div className="flex flex-col items-center px-6 md:px-16 lg:px-24 lx:px-32 pt-20 pb-30">
      ...
      <div>
        {exclusiveOffers.map((item)=>(
            <div key={item._id} className="group relative flex flex-col items-start justofy-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center" style={{backgroundImage:`url(${item.image})`}}>
                <p className="px-3 py-1 absolute top-4 left-4 text-xs bg-white text-greay-800 font-medium rounded-full">{item.priceOff} % OFF</p>
                <div >
